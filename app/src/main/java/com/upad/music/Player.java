...
public class Player {
    ...
    public void init(List<Music> musics) {
                ...
                switch (PLAY_MODE) {
                    case PlayMode.PLAY_MODE_LIST:
                        if (currentIndex == mSize - 1) {
                            ...
                            currentIndex++;
                            LogUtil.Log("setOnCompletionListener-currentIndex");
                        }
                        break;
        ...
        mPlayer.setOnErrorListener((mp, what, extra) -> {
            showToastl(APP.getContext(), "抱歉，当前资源找不到，即将播放下一个");
            Log.e(TAG, "onError: " + what + "," + extra);
            LogUtil.Log("抱歉，当前资源找不到，即将播放下一个");
            next();
            return false;
        });
        setAndPrepared();
    ...
    public boolean prev() {
        LogUtil.Log("Player-prev-index:"+currentIndex+"; size:"+musics.size()+"; PLAY_MODE:"+PLAY_MODE);
        if (mPlayer != null) {
            switch (PLAY_MODE) {
                case PlayMode.PLAY_MODE_LIST:
                    if (currentIndex == 0) {
    ...
    public boolean next() {
        LogUtil.Log("Player-next-index:"+currentIndex+"; size:"+musics.size()+"; PLAY_MODE:"+PLAY_MODE);
        if (mPlayer != null) {
            switch (PLAY_MODE) {
                case PlayMode.PLAY_MODE_LIST:
                    if (currentIndex == mSize - 1) {
                        ...
                        currentIndex++;
                        LogUtil.Log("Player-next-index11:"+currentIndex);
                    }
                    break;
                case PlayMode.PLAY_MODE_REPEAT_ONE:
                case PlayMode.PLAY_MODE_REPEAT_LIST:
                    if (currentIndex == mSize - 1) {
                        ...
                        currentIndex++;
                        LogUtil.Log("Player-next-index22:"+currentIndex);
                    }
                    break;
                case PlayMode.PLAY_MODE_SHUFFLE:
                    int tempIndex = currentIndex;
